...
x_com_pend = x_cart + r_com_pend*cos(theta_pend);
y_com_pend = r_com_pend*sin(theta_pend);

x_tip_pend = x_cart + l_pend*cos(theta_pend);
y_tip_pend = l_pend*sin(theta_pend);
...
ke_cart = 0.5*m_cart*(dx_com_cart^2);
ke_pend = 0.5*m_pend*(dx_com_pend^2 + dy_com_pend^2) + ...
    0.5*I_pend*dtheta_pend^2;
...
x_cart_eq = 0;
theta_pend_eq = sym(pi/2); % theta_pend is defined ccw+ w.r.t. x-axis
dx_cart_eq = 0;
dtheta_pend_eq = 0;
...
x_eq = [x_cart_eq; theta_pend_eq; dx_cart_eq; dtheta_pend_eq];
...
% control Hamiltonian:
H = simplify(running_cost + lambda'*(f_ss + g_ss*u));

% create a MATLAB function for the control Hamiltonian, since H(T) = 0 will
% provide a boundary condition for the TPBVP and therefore must be called
% by tpbvp_bc.m:
matlabFunction(H,'File','autogen_control_hamiltonian');
...
% Export u_PMP to a MATLAB function:
matlabFunction(u_PMP,'File','autogen_control_PMP');
